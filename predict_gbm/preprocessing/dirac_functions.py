...
def create_image_pyramid(image, down_sample_factor, device, channel=1):
    ...
    for level in down_sample_factor:
        ...
        down_image = F.conv3d(
            image, kernel, stride=level, padding=padding, groups=channel
        )
...
class Dataset_epoch(Data.Dataset):
    ...
    def __getitem__(self, step):
        ...
        if self.norm:
            return (
                torch.from_numpy(imgnorm(moving_img)).float(),
                torch.from_numpy(imgnorm(fixed_img)).float(),
            )
        else:
            return (
                torch.from_numpy(moving_img).float(),
                torch.from_numpy(fixed_img).float(),
            )
...
class Dataset_epoch_NLST(Data.Dataset):
    ...
    def __getitem__(self, step):
        ...
        keypoints = []
        with open(self.keypoint_csv[step][0], newline="") as fixed_csvfile, open(
            self.keypoint_csv[step][1], newline=""
        ) as moving_csvfile:
        ...
        if self.need_label:
            ...
            return (
                torch.from_numpy(imgnorm(moving_img)).float(),
                torch.from_numpy(imgnorm(fixed_img)).float(),
                torch.from_numpy(moving_mask).float(),
                torch.from_numpy(fixed_mask).float(),
                keypoints,
            )
        else:
            return (
...
class Dataset_bratsreg_bidirection_all(Data.Dataset):
    ...
    def __getitem__(self, step):
        ...
        fixed_img = np.concatenate(
            (fixed_img_t1ce, fixed_img_flair, fixed_img_t2), axis=0
        )
        moving_img = np.concatenate(
            (moving_img_t1ce, moving_img_flair, moving_img_t2), axis=0
        )
...
class Dataset_bratsreg_bidirection_all4(Data.Dataset):
    ...
    def __getitem__(self, step):
        ...
        fixed_img = np.concatenate(
            (fixed_img_t1, fixed_img_t1ce, fixed_img_flair, fixed_img_t2), axis=0
        )
        moving_img = np.concatenate(
            (moving_img_t1, moving_img_t1ce, moving_img_flair, moving_img_t2), axis=0
        )
    ...
    def __getitem__(self, step):
        ...
        if self.norm:
            return (
                torch.from_numpy(imgnorm(moving_img)).float(),
                torch.from_numpy(imgnorm(fixed_img)).float(),
                torch.from_numpy(tumor_img).float(),
            )
        else:
            return (
                torch.from_numpy(moving_img).float(),
    ...
    def __getitem__(self, step):
        ...
        if self.norm:
            return (
                torch.from_numpy(imgnorm(moving_img)).float(),
                torch.from_numpy(imgnorm(fixed_img)).float(),
                torch.from_numpy(reg_seg).float(),
            )
        else:
            return (
                torch.from_numpy(moving_img).float(),
...
class Validation_Brats_all(Data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        fixed_img = np.concatenate(
            (fixed_t1ce_img, fixed_flair_img, fixed_t2_img), axis=0
        )
        moved_img = np.concatenate(
            (moved_t1ce_img, moved_flair_img, moved_t2_img), axis=0
        )
        ...
        for mov_line, fixed_line in zip(moved_reader, fixed_reader):
            moved_key_list.append(
                [float(mov_line[1]), float(mov_line[2]) + 239.0, float(mov_line[3])]
            )
            fixed_key_list.append(
                [
                    float(fixed_line[1]),
                    float(fixed_line[2]) + 239.0,
                    float(fixed_line[3]),
                ]
            )
        ...
        output = {
            "fixed": fixed_img.float(),
            "move": moved_img.float(),
            "fixed_label": np.array(fixed_key_list),
            "move_label": np.array(moved_key_list),
            "index": index,
        }
        return output
...
class Validation_Brats_all4(Data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        fixed_img = np.concatenate(
            (fixed_t1_img, fixed_t1ce_img, fixed_flair_img, fixed_t2_img), axis=0
        )
        moved_img = np.concatenate(
            (moved_t1_img, moved_t1ce_img, moved_flair_img, moved_t2_img), axis=0
        )
        ...
        for mov_line, fixed_line in zip(moved_reader, fixed_reader):
            moved_key_list.append(
                [
                    ...
                    -1.0 * float(mov_line[2]),
                    float(mov_line[3]),
                ]
            )
            fixed_key_list.append(
                [
                    239.0 - float(fixed_line[1]),
                    -1.0 * float(fixed_line[2]),
                    float(fixed_line[3]),
                ]
            )
...
class Validation_Brats_with_mask(Data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        output = {
            ...
            "fixed_label": np.array(fixed_key_list),
            "move_label": np.array(moved_key_list),
            "tumor_mask": torch.from_numpy(tumor_img).float(),
            "index": index,
        }
        return output
...
class Validation_DIRLab(Data.Dataset):
    ...
    def __init__(
        self, move_list, fixed_list, move_keypoint, fixed_keypoint, need_label=True
    ):
        "Initialization"
    ...
    def __getitem__(self, step):
        ...
        if self.need_label:
            ...
            return (
                torch.from_numpy(imgnorm(insp_img)).float(),
                torch.from_numpy(imgnorm(exp_img)).float(),
                torch.from_numpy(insp_mask).float(),
                torch.from_numpy(exp_mask).float(),
                keypoints,
            )
        else:
            return (
